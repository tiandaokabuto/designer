...
import { FormOutlined, BuildOutlined, FileAddOutlined, SkinOutlined } from '@ant-design/icons';
import { uniqueId } from 'lodash';

import { SUBMIT_COMPONENT, CANCEL_COMPONENT } from './components/componentTypes';
...
const PanelRight = props => {
    let { dataList, focusItemId, handleChange, layout, setLayout, findPosition, device } = props;
            ...
            case 'selectedType':
                return (
                    <Radio.Group
                        ...
                        buttonStyle="solid"
                        size="small"
                        options={NAME_MAP.options.find(item => item.key === 'selectedType').values}
                    />
                );
            case 'width':
                return (
    ...
    const getInputs = typeList => {
        return typeList.map((item, index) => {
            if (Object.keys(focusItem.attribute).indexOf(item.key) >= 0) {
                ...
                return (
                    <div className="parampanel-item" key={`${item.name}_${index}`}>
    ...
    return (
        <Fragment>
            <div className="panel-right-config" style={{ textAlign: 'center', height: 38 }}>
                参数配置面板
            </div>
