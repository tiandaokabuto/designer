...
const PanelRight = props => {
  ...
  const NAME_MAP = {
    ...
    layout: [{ key: 'width', name: '组件宽度', values: [25, 50, 75, 100] }],
  };
  ...
  const getInputs = typeList => {
    return typeList.map((item, index) => {
      if (Object.keys(focusItem.attribute).indexOf(item.key) >= 0) {
        ...
        return (
          <div className="parampanel-item" key={_uniqueId()}>
            <span style={{ paddingRight: 20 }}>{item.name}</span>

            {/** 用对应类型的配置参数.attribute 生成配置填写框 */}
            {getComponentType(focusItem.attribute, item.key)}
          </div>
        );
      }
    });
